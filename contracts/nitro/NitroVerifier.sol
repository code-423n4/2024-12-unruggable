...
contract NitroVerifier is AbstractVerifier {
    ...
    function getStorageValues(
        ...
        bytes32 stateRoot = _verifyStateRoot(node.confirmData, p.rlpEncodedBlock, p.sendRoot);
        return GatewayVM.evalRequest(req, ProofSequence(0, stateRoot, p.proofs, p.order, _hooks));
    }

    function _verifyNode(bytes memory context, uint64 nodeNum) internal view returns (Node memory node) {
    ...
    function _verifyStateRoot(
        ...
        bytes32 computed = keccak256(abi.encodePacked(keccak256(rlpEncodedBlock), sendRoot));
