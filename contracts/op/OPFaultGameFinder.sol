...
interface IDisputeGameFactory {
    ...
    function gameAtIndex(
        uint256 index
    ) external view returns (uint256 gameType, uint256 created, IDisputeGame gameProxy);
}
...
contract OPFaultGameFinder {
    ...
    function findGameIndex(
        ...
        while (gameCount > 0) {
            (uint256 gameType, uint256 created, IDisputeGame gameProxy) = factory.gameAtIndex(--gameCount);
            if (_isGameUsable(gameProxy, gameType, created, gameTypeBitMask, minAgeSec)) {
                return gameCount;
            }
        }
        revert GameNotFound();
    ...
    function _isGameUsable(
        ...
        if (minAgeSec == 0) {
            ...
            return created <= block.timestamp - minAgeSec && gameProxy.status() != CHALLENGER_WINS;
        }
    }
}
