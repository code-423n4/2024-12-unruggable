...
interface IOPFaultGameFinder {
    ...
    function gameAtIndex(
        ...
        uint256 gameTypeBitMask,
        uint256 gameIndex
    ) external view returns (uint256 gameType, uint256 created, IDisputeGame gameProxy, uint256 l2BlockNumber);
}
...
contract OPFaultVerifier is AbstractVerifier {
    ...
    function getLatestContext() external view virtual returns (bytes memory) {
        return abi.encode(_gameFinder.findGameIndex(_portal, _minAgeSec, _gameTypeBitMask, 0));
    }
    ...
    function getStorageValues(
        ...
        require(
            Claim.unwrap(gameProxy.rootClaim()) == Hashing.hashOutputRootProof(p.outputRootProof),
            "OPFault: rootClaim"
        );
