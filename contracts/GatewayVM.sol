...
library GatewayVM {
    ...
    function stackAsUint256(Machine memory vm, uint256 i) internal pure returns (uint256 x) {
        x = vm.stack[i];
        if (!vm.isStackRaw(i)) {
            uint256 n;
            assembly {
    ...
    function popAsBytes(Machine memory vm) internal pure returns (bytes memory) {
    ...
    function readUint(Machine memory vm, uint256 n) internal pure returns (uint256 x) {
        if (n == 0) return 0;
    ...
    function readBytes(Machine memory vm, uint256 n) internal pure returns (bytes memory v) {
    ...
    function evalCommand(Machine memory vm, bytes[] memory outputs) internal view returns (uint8 /*exitCode*/) {
        while (vm.pos < vm.buf.length) {
            ...
            if (op <= GatewayOP.PUSH_32) {
                ...
                vm.storageRoot = vm.proofs.hooks.verifyAccountState(vm.proofs.stateRoot, vm.target, vm.readProof());
                vm.slot = 0;
                ...
                uint256 i = vm.pop();
                if (vm.isStackRaw(i)) {
                    vm.pushUint256(uint256(smallKeccak(bytes32(vm.stackAsUint256(i)))));
                } else {
                    vm.pushUint256(uint256(keccak256(vm.stackAsBytes(i))));
                }
            } else if (op == GatewayOP.LENGTH) {
                uint256 i = vm.pop();
                vm.pushUint256(vm.isStackRaw(i) ? 32 : vm.stackAsBytes(i).length);
                ...
                while (count > 0) {
                    ...
                    if (
                        (flags &
                            (vm2.evalCommand(outputs) != 0 ? EvalFlag.STOP_ON_FAILURE : EvalFlag.STOP_ON_SUCCESS)) != 0
                    ) {
