...
contract ZKSyncVerifier is AbstractVerifier {
    ...
    function getStorageValues(
        ...
        StoredBatchInfo memory batchInfo = abi.decode(p.encodedBatch, (StoredBatchInfo));
        _checkWindow(batchIndex1, batchInfo.batchNumber);
        require(keccak256(p.encodedBatch) == _diamond.storedBatchHash(batchInfo.batchNumber), "ZKS: batchHash");
        require(batchInfo.l2LogsTreeRoot == _diamond.l2LogsRootHash(batchInfo.batchNumber), "ZKS: l2LogsRootHash");
        return GatewayVM.evalRequest(req, ProofSequence(0, batchInfo.batchHash, p.proofs, p.order, _hooks));
    }
}
