...
interface IRootChain {
    ...
    function headerBlocks(
        uint256
    ) external view returns (bytes32 root, uint256 start, uint256 end, uint256 createdAt, address proposer);
}
...
contract PolygonPoSVerifier is AbstractVerifier {
    ...
    function getStorageValues(
        ...
        (bytes32 rootHash, uint256 l2BlockNumberStart, , uint256 createdAt, ) = _rootChain.headerBlocks(headerBlock);
        require(rootHash != bytes32(0), "PolygonPoS: checkpoint");
        if (headerBlock1 != headerBlock) {
            (, , , uint256 createdAt1, ) = _rootChain.headerBlocks(headerBlock1);
