...
contract TaikoVerifier is AbstractVerifier {
    ...
    function getStorageValues(
        ...
        TransitionState memory ts = _rollup.getTransition(p.blockId, p.parentHash); // reverts if invalid
        return GatewayVM.evalRequest(req, ProofSequence(0, ts.stateRoot, p.proofs, p.order, _hooks));
    }
}
