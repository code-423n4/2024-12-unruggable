...
contract LineaVerifierHooks is IVerifierHooks {
    ...
    function verifyAccountState(
        ...
        if (right.nodes.length == 0) {
            _requireInclusion(stateRoot, hKey, SparseMerkleProof.hashAccountValue(proof.value), proof);
    ...
    function verifyStorageValue(
        ...
        if (right.nodes.length == 0) {
            ...
            _requireInclusion(storageRoot, hKey, SparseMerkleProof.hashStorageValue(value), proof);
            return value;
        } else {
    ...
    // 20240917: 1.3m gas
    function _requireInclusion(bytes32 root, bytes32 hKey, bytes32 hValue, Proof memory proof) internal pure {
    ...
    function _requireExclusion(bytes32 root, bytes32 hKey, Proof memory proofL, Proof memory proofR) internal pure {
