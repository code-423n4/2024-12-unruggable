...
contract UnfinalizedLineaVerifier is AbstractVerifier {
    ...
    function getStorageValues(
        ...
        uint256 l2BlockNumber = _rollup.shnarfFinalBlockNumbers(keccak256(p.abiEncodedTuple));
        ...
        if (l2BlockNumber < _rollup.currentL2BlockNumber()) {
            ...
            require(stateRoot == _rollup.stateRootHashes(l2BlockNumber), "UnfinalizedLinea: not finalized");
        }
        return GatewayVM.evalRequest(req, ProofSequence(0, stateRoot, p.proofs, p.order, _hooks));
    }

    function _extractStateRoot(bytes memory v) internal pure returns (bytes32 stateRoot) {
        assembly {
