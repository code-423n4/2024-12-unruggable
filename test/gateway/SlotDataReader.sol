...
contract SlotDataReader is GatewayFetchTarget, Ownable {
    ...
    function readSlot(uint256 slot) public view returns (uint256) {
        GatewayRequest memory r = GatewayFetcher.newRequest(1).setTarget(_target).setSlot(slot).read().setOutput(0);
    ...
    function readName() external view returns (string memory) {
        GatewayRequest memory r = GatewayFetcher.newRequest(1).setTarget(_target).setSlot(1).readBytes().setOutput(0);
    ...
    function readZero() external view returns (uint256) {
        GatewayRequest memory r = GatewayFetcher.newRequest(1).setTarget(_target).setSlot(5).read().setOutput(0);
        fetch(_verifier, r, this.uint256Callback.selector);
    }

    function readRootStr(string[] memory keys) external view returns (string memory) {
        GatewayRequest memory r = GatewayFetcher.newRequest(1).setTarget(_target).setSlot(12);
