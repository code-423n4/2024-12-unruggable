...
interface IExtendedResolver {
    function resolve(bytes memory, bytes memory) external view returns (bytes memory);
}
...
contract L1Resolver is GatewayFetchTarget, IExtendedResolver, IERC165 {
    ...
    function leadingLabelhash(bytes calldata name) internal pure returns (bytes32) {
