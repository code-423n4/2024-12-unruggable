...
contract TestGatewayFetcher is Test {
    ...
    function test_stack() external {
        ...
        unchecked {
            evalUint256(R(1).push(2).push(3).dup2().pow().pow().pow(), 2 ** uint256(3 ** uint256(2 ** 3)));
        }
        vm.expectRevert();
        evalRequest(R(1).dup());
        vm.expectRevert();
    ...
    function test_eval_setOutput() external view {
        evalUint256(R(1).push(C().push(1).setOutput(0)).eval().pushOutput(0), 1);
    }
    ...
    function evalRequest(GatewayRequest memory r) internal view returns (bytes[] memory outputs, uint8 exitcode) {
